...
#    imc, contours,hierarchy = cv2.findContours(mask.copy(),cv2.RETR_EXTERNAL, 2)
    ...
    imc, contours, hierarchy = cv2.findContours(fgMask.copy(), cv2.RETR_EXTERNAL, 2)
    cnt=ut.getMaxContour(contours,4000)                          # using contours to capture the skin filtered image of the hand
    if cnt is not None:
        gesture,label=ut.getGestureImg(cnt,img1,fgMask,model)   # passing the trained model for prediction and fetching the result
